...
BWD_SWEEP_1:	render_a(bwd_sweep_1, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
BWD_SWEEP_2:	h_decrement(CHAR_POS, 24, 0, t0, t1)
		load_char_pos(s1, s2)
...
#########################################
HIGH_BK_KICK:	la t1,CHAR_ATTACK
		lbu t0,1(t1)		# current sprite
...
HIGH_BK_KICK_0:	render_a(high_bk_kick_1, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
HIGH_BK_KICK_1:	h_decrement(CHAR_POS, 16, 0, t0, t1)
		load_char_pos(s1, s2)
...
HIGH_BK_KICK_3:	render_a(high_bk_kick_2, s1, s2, 56, 56, s0, zero, zero)
		h_increment(CHAR_POS, 16, 0, t0, t1)
		j ATTACK_ANIM_E

#########################################
#	FLYING KICK MOVEMENT		#
#########################################
FLYING_KICK:	la t1,CHAR_ATTACK
		lbu t0,1(t1)		# current sprite
...
FLYING_KICK_0:	render_a(kick, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
FLYING_KICK_1:	render_a(flying_kick_1, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
FLYING_KICK_2:	h_decrement(CHAR_POS, 8, 2, t0, t1)	# y -= 8
		load_char_pos(s1, s2)
		render_a(flying_kick_1, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
FLYING_KICK_3:	h_increment(CHAR_POS, 8, 0, t0, t1)	# x += 8
		h_decrement(CHAR_POS, 16, 2, t0, t1)	# y -= 16
		load_char_pos(s1, s2)
...
FLYING_KICK_5:	h_increment(CHAR_POS, 8, 2, t0, t1)	# y += 8
		...
		render_a(flying_kick_2, s1, s2, 64, 56, s0, zero, zero)
		j ATTACK_ANIM_E
FLYING_KICK_6:	h_increment(CHAR_POS, 16, 2, t0, t1)	# y += 16
		load_char_pos(s1, s2)
		render_a(flying_kick_1, s1, s2, 48, 56, s0, zero, zero)
		j ATTACK_ANIM_E
...
# Move left 		(press a)
RI_MOVE_LEFT:	lb t0,CHAR_ATTACK
		bnez t0,REC_INPUT_CLN
...
RI_SJ_KICK:	register_attack(2)
# Forward sweep		(press X, shift + x)
RI_FWD_SWEEP:	register_attack(3)
# Backwards sweep	(press Z, shift + z)
RI_BWD_SWEEP:	register_attack(4)
# Roundhouse		(press A, shift + a)
RI_ROUNDHOUSE:	register_attack(5)
# High back kick	(press Q, shift + q)
RI_HIGH_B_KICK:	register_attack(6)
# Fling kick		(press W, shift + w)
RI_FLYING_KICK:	register_attack(7)
